...
@Component({
  ...
  standalone: true,
  imports: [ArticlesFeedComponent, NgClass, RouterModule],
...
export class ProfileComponent {
  ...
  private _loadProfile(): void {
    ...
    this._constructUserProfileRequest().subscribe({
  ...
  private _constructUserProfileRequest(): Observable<ProfileResponse> {
    return this._authService.currentUser$.pipe(
      switchMap((user) => {
        ...
        return this._profileService
          .getProfile(this._profileUsername)
          .pipe(finalize(() => this.isLoading.set(false)));
      })
    );
  }
  ...
  public follow(): void {
    this._profileService.followUser(this.profile()!.username).subscribe({
      next: (profile: ProfileResponse) => {
        this.profile.set(profile.profile);
      },
      error: () => {
  ...
  public unfollow(): void {
    this._profileService.unfollowUser(this.profile()!.username).subscribe({
      next: (profile: ProfileResponse) => {
        this.profile.set(profile.profile);
      },
      error: () => {
